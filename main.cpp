...
void castWindow3D(const vector<vector<vector<block>>> &arr) {
  SetConfigFlags(FLAG_VSYNC_HINT);
  SetConfigFlags(FLAG_MSAA_4X_HINT);
  InitWindow(1000, 800, "Viviore 0.0.1");
  SetTargetFPS(30);
      ...
      for (int j = 0; j < y; j++)
        for (int k = 0; k < z; k++) {
          if (arr[i][j][k].isOre) {
            ...
            DrawCube({(float)i - cx, (float)j - cy, (float)k - cz}, 1.0f, 1.0f,
                     1.0f, oreColor);
...
void castWindow2D(const vector<vector<vector<block>>> &arr, Axis &axis,
                  int &slice) {
  SetConfigFlags(FLAG_VSYNC_HINT);
  SetConfigFlags(FLAG_MSAA_4X_HINT);
  ...
  while (!WindowShouldClose()) {
    ...
    int maxSlice = axis == X ? x : axis == Y ? y : z;

    // Rectangle viewport = {20, GetScreenHeight() - 180, 160, 160};
    Rectangle viewport = {920, 600, 240, 180};
    ...
    // float px = (float)slice / (x - 1) * 4 - 1;
    // float py = (float)slice / (y - 1) * 4 - 1;
    // float pz = (float)slice / (z - 1) * 4 - 1;

    float p = (float)slice / (maxSlice - 1) * 4 - 2;
...
int main() {
  ...
  exportOre(litho);
  // castWindow3D(litho);
  castWindow2D(litho, currentAxis, slice);
